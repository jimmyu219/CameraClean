...
volatile int * HPS_CLK2			= (int *) 0xFF200100;
...
int main(void){
	...
	*oDigits = 0;
	...
	while(1)
	{
		...
		//reset col sum and row sum to 0
		//rowsSumArr[480] = { 0 };
		//colsSumArr[640] = { 0 };
		...
		*oState = 7;				// State 3 - Detect projector
		...
		{
			printf("\nThe sum of the cols for col[%d]is: %d\n", cols, colsSumArr[cols]);
...
/*					for (k = 0; k < 784; k++)
					{
						if(digitArr[k])
						{
							//sum += W1[i][2*28+2+28* ((int)k/24)+k]+k%24;
